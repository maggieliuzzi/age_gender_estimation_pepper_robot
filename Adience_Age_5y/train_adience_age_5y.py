...
train_path = os.path.join(home_path, "dataset_adience_age_5y/train")
validate_path = os.path.join(home_path, "dataset_adience_age_5y/validate")
test_path = os.path.join(home_path, "dataset_adience_age_5y/test")
...
model_new.add(layers.Dense(12, activation='softmax')) # + model_new.add(layers.LeakyReLU(alpha=0.25)) ?
model_new.summary()
...
model_new.fit_generator(train_generator, epochs=10, steps_per_epoch=len(train_generator), verbose=1,
                        validation_data=validate_generator, validation_steps=len(validate_generator))
model_new.save('final_model_adience_age_5YearSlots.h5')
